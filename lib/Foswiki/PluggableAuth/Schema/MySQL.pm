...
package Foswiki::PluggableAuth::Schema::MySQL;

=begin TML

---+ package Foswiki::PluggableAuth::Schema:;MySQL

Database schema when using a <nop>MySQL
...
sub getDefinition {
  ...
  my @schema = ([
    ...
    "CREATE UNIQUE INDEX %prefix%idx_users_loginName ON %prefix%users (loginName, pid)",
    "CREATE UNIQUE INDEX %prefix%idx_users_wikiName ON %prefix%users (wikiName)",
    ...
    "CREATE UNIQUE INDEX %prefix%idx_passwords_uid ON %prefix%passwords (uid)",
    ...
    "CREATE UNIQUE INDEX %prefix%idx_groups_name ON %prefix%groups (wikiName)",

    "CREATE TABLE IF NOT EXISTS %prefix%group_members (
            gid VARCHAR(255) NOT NULL,
            mid VARCHAR(255) NOT NULL
    ) DEFAULT CHARSET=utf8mb4 DEFAULT COLLATE utf8mb4_bin",
    "CREATE INDEX %prefix%idx_group_members_gid ON %prefix%group_members (gid)",
    "CREATE INDEX %prefix%idx_group_members_mid ON %prefix%group_members (mid)",
    "CREATE UNIQUE INDEX %prefix%idx_group_members ON %prefix%group_members (gid, mid)",
    ...
    "CREATE INDEX %prefix%idx_user_keys_uid ON %prefix%user_keys (uid)",
    "CREATE TABLE IF NOT EXISTS %prefix%providers (
            id VARCHAR(255) NOT NULL PRIMARY KEY,
            name VARCHAR(255) NOT NULL,
            enabled INTEGER DEFAULT 0
    ) DEFAULT CHARSET=utf8mb4 DEFAULT COLLATE utf8mb4_bin",
    "CREATE INDEX %prefix%idx_providers_name ON %prefix%providers (name)",
  ], [

...
package PluggableAuthMastodonProviderTests;
...
use Foswiki::PluggableAuth::Provider::Mastodon ();
use Error qw(:try);
...
sub set_up {
  ...
  $Foswiki::cfg{PluggableAuth}{Providers}{Mastodon}{Enabled} = 1;
  $this->{provider} = $this->{auth}->getProvider("Mastodon");
}
...
sub test_self {
  ...
  $this->assert($this->{provider}->isa("Foswiki::PluggableAuth::Provider::Mastodon"));
}
...
sub test_id {
  ...
  $this->assert_equals("Mastodon", $pid);
}

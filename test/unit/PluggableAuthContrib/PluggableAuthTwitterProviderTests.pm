...
package PluggableAuthTwitterProviderTests;
...
use Foswiki::PluggableAuth::Provider::Twitter ();
use Error qw(:try);
...
sub set_up {
  ...
  $Foswiki::cfg{PluggableAuth}{Providers}{Twitter}{Enabled} = 1;
  $this->{provider} = $this->{auth}->getProvider("Twitter");
}
...
sub test_self {
  ...
  $this->assert($this->{provider}->isa("Foswiki::PluggableAuth::Provider::Twitter"));
}
...
sub test_id {
  ...
  $this->assert_equals("Twitter", $pid);
}

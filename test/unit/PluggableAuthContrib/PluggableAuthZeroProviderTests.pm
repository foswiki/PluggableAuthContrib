...
package PluggableAuthZeroProviderTests;
...
use Foswiki::PluggableAuth::Provider::AuthZero ();
use Error qw(:try);
...
sub set_up {
  ...
  $Foswiki::cfg{PluggableAuth}{Providers}{AuthZero}{Enabled} = 1;
  $this->{provider} = $this->{auth}->getProvider("AuthZero");
}
...
sub test_self {
  ...
  $this->assert($this->{provider}->isa("Foswiki::PluggableAuth::Provider::AuthZero"));
}
...
sub test_id {
  ...
  $this->assert_equals("AuthZero", $pid);
}

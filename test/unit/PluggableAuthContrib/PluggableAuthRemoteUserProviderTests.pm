...
package PluggableAuthRemoteUserProviderTests;
...
use Foswiki::PluggableAuth::Provider::RemoteUser ();
use Error qw(:try);
...
sub set_up {
  ...
  $Foswiki::cfg{PluggableAuth}{Providers}{RemoteUser}{Enabled} = 1;
  $this->{provider} = $this->{auth}->getProvider("RemoteUser");
}
...
sub test_self {
  ...
  $this->assert($this->{provider}->isa("Foswiki::PluggableAuth::Provider::RemoteUser"));
}
...
sub test_id {
  ...
  $this->assert_equals("RemoteUser", $pid);
}

...
package PluggableAuthSlackProviderTests;
...
use Foswiki::PluggableAuth::Provider::Slack ();
use Error qw(:try);
...
sub set_up {
  ...
  $Foswiki::cfg{PluggableAuth}{Providers}{Slack}{Enabled} = 1;
  $this->{provider} = $this->{auth}->getProvider("Slack");
}
...
sub test_self {
  ...
  $this->assert($this->{provider}->isa("Foswiki::PluggableAuth::Provider::Slack"));
}

...
package PluggableAuthYahooProviderTests;
...
use Foswiki::PluggableAuth::Provider::Yahoo ();
use Error qw(:try);
...
sub set_up {
  ...
  $Foswiki::cfg{PluggableAuth}{Providers}{Yahoo}{Enabled} = 1;
  $this->{provider} = $this->{auth}->getProvider("Yahoo");
}
...
sub test_self {
  ...
  $this->assert($this->{provider}->isa("Foswiki::PluggableAuth::Provider::Yahoo"));
}

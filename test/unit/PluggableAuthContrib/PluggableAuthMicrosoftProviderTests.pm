...
package PluggableAuthMicrosoftProviderTests;
...
use Foswiki::PluggableAuth::Provider::Microsoft ();
use Error qw(:try);
...
sub set_up {
  ...
  $Foswiki::cfg{PluggableAuth}{Providers}{Microsoft}{Enabled} = 1;
  $this->{provider} = $this->{auth}->getProvider("Microsoft");
}
...
sub test_self {
  ...
  $this->assert($this->{provider}->isa("Foswiki::PluggableAuth::Provider::Microsoft"));
}
...
sub test_id {
  ...
  $this->assert_equals("Microsoft", $pid);
}

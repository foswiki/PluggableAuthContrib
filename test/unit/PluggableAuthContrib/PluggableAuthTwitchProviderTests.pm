...
package PluggableAuthTwitchProviderTests;
...
use Foswiki::PluggableAuth::Provider::Twitch ();
use Error qw(:try);
...
sub set_up {
  ...
  $Foswiki::cfg{PluggableAuth}{Providers}{Twitch}{Enabled} = 1;
  $this->{provider} = $this->{auth}->getProvider("Twitch");
}
...
sub test_self {
  ...
  $this->assert($this->{provider}->isa("Foswiki::PluggableAuth::Provider::Twitch"));
}

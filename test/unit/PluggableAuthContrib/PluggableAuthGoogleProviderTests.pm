...
package PluggableAuthGoogleProviderTests;
...
use Foswiki::PluggableAuth::Provider::Google ();
use Error qw(:try);
...
sub set_up {
  ...
  $Foswiki::cfg{PluggableAuth}{Providers}{Google}{Enabled} = 1;
  $this->{provider} = $this->{auth}->getProvider("Google");
}
...
sub test_self {
  ...
  $this->assert($this->{provider}->isa("Foswiki::PluggableAuth::Provider::Google"));
}
...
sub test_id {
  ...
  $this->assert_equals("Google", $pid);
}

sub test_canSetPassword {
  my $this = shift;

  $this->assert_equals(0, $this->{provider}->canSetPassword(), "provider cannot set any password");
}

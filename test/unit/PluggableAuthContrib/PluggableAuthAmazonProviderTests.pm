...
package PluggableAuthAmazonProviderTests;
...
use Foswiki::PluggableAuth::Provider::Amazon ();
use Error qw(:try);
...
sub set_up {
  ...
  $Foswiki::cfg{PluggableAuth}{Providers}{Amazon}{Enabled} = 1;
  $this->{provider} = $this->{auth}->getProvider("Amazon");
}
...
sub test_self {
  ...
  $this->assert($this->{provider}->isa("Foswiki::PluggableAuth::Provider::Amazon"));
}

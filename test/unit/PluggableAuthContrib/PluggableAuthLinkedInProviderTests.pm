...
package PluggableAuthLinkedInProviderTests;
...
use Foswiki::PluggableAuth::Provider::LinkedIn ();
use Error qw(:try);
...
sub set_up {
  ...
  $Foswiki::cfg{PluggableAuth}{Providers}{LinkedIn}{Enabled} = 1;
  $this->{provider} = $this->{auth}->getProvider("LinkedIn");
}
...
sub test_self {
  ...
  $this->assert($this->{provider}->isa("Foswiki::PluggableAuth::Provider::LinkedIn"));
}
...
sub test_id {
  ...
  $this->assert_equals("LinkedIn", $pid);
}

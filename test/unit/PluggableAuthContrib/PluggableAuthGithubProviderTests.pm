...
package PluggableAuthGithubProviderTests;
...
use Foswiki::PluggableAuth::Provider::Github ();
use Error qw(:try);
...
sub set_up {
  ...
  $Foswiki::cfg{PluggableAuth}{Providers}{Github}{Enabled} = 1;
  $this->{provider} = $this->{auth}->getProvider("Github");
}
...
sub test_self {
  ...
  $this->assert($this->{provider}->isa("Foswiki::PluggableAuth::Provider::Github"));
}

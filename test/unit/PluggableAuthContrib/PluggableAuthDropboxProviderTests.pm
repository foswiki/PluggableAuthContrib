...
package PluggableAuthDropboxProviderTests;
...
use Foswiki::PluggableAuth::Provider::Dropbox ();
use Error qw(:try);
...
sub set_up {
  ...
  $Foswiki::cfg{PluggableAuth}{Providers}{Dropbox}{Enabled} = 1;
  $this->{provider} = $this->{auth}->getProvider("Dropbox");
}
...
sub test_self {
  ...
  $this->assert($this->{provider}->isa("Foswiki::PluggableAuth::Provider::Dropbox"));
}
...
sub test_id {
  ...
  $this->assert_equals("Dropbox", $pid);
}
